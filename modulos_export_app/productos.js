...
class productos {
    // constructor
    constructor(id, nombre, tipo, color, cantidad, medida, costo_unitario, Proveedor, favorito, vendido, en_espera, no_vendido) {
        this.Id = id;
        this.Nombre = nombre;
        this.Tipo = tipo;
        this.Color = color;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (error) {
                ...
                for (let index = 0; index < result.length; index++) {
                    function return_the_truth(resultado) {
                        if (resultado === 0) {
                            return false;
                        }
                        else {
                    ...
                    var nuevo_prod = new productos(result[index].id, result[index].nombre, result[index].tipo, result[index].color, result[index].cantidad, result[index].medida, result[index].costo_unitario, result[index].proveedor, return_the_truth(result[index].favorito), return_the_truth(result[index].vendido), return_the_truth(result[index].en_espera), return_the_truth(result[index].no_vendido));
...
function UPDATE_marks(number1, number2, number3, number4, ID) {
    let prod = array_productos.find(element => element.Id == parseInt(ID));
    console.log(array_productos);
    console.log(number1, number2, number3, number4, ID);
    if ((prod === null || prod === void 0 ? void 0 : prod.Favorito) == true && number1 == 1 || (prod === null || prod === void 0 ? void 0 : prod.Vendido) == true && number2 == 1 || (prod === null || prod === void 0 ? void 0 : prod.En_espera) == true && number3 == 1 || (prod === null || prod === void 0 ? void 0 : prod.No_vendido) == true && number4 == 1) {
        // call 
        connect.query(`UPDATE productos SET favorito = ?, vendido = ?, en_espera = ?, no_vendido = ? WHERE id = ?`, [0, 0, 0, 0, ID], (error, results, fields) => {
            if (error) {
                throw error;
            }
            else {
        ...
        // call
        connect.query(`UPDATE productos SET favorito = ?, vendido = ?, en_espera = ?, no_vendido = ? WHERE id = ?`, [number1, number2, number3, number4, ID], (error, results, fields) => {
            if (error) {
                throw error;
            }
            else {
...
function create_update_proveedores(SPAN, string_or_number, producto, ID) {
    const body = document.getElementsByTagName("body")[0];
    let create_window = document.createElement("div");
    body.appendChild(create_window);
    create_window.className = "update_window";
    let span = document.createElement("span");
    let paragraph = document.createElement("p");
    let update_inputs = document.createElement("select");
    let modificar = document.createElement("button");
    let cancelar = document.createElement("button");
    // create window append child
    create_window.appendChild(span);
    ...
    connect.query('SELECT nombre FROM proveedores', function (error, results, fields) {
        if (error) {
            throw error;
        }
        else {
...
let array_btns_search = ["tipo", "color", "proveedor", "favorito", "vendido", "en espera", "no vendido"];
let array_main_div = ["nombre", "tipo", "color", "cantidad", "medida", "costo unitario", "costo total", "proveedor"];
...
search_input.addEventListener("dblclick", function (e) {
    ...
    if (body.lastElementChild === body_last_element) {
        ...
        header === null || header === void 0 ? void 0 : header.appendChild(container_tag);
        ...
        search_input.addEventListener("keypress", function (e) {
            if (e.key === "Enter") {
                ...
                for (let index = 0; index < array_new_productos.length; index++) {
                    let producto_element = array_new_productos[index];
        ...
        search_input.style.width = "50vw";
        search_input.style.left = "3%";
        (_a = header === null || header === void 0 ? void 0 : header.lastElementChild) === null || _a === void 0 ? void 0 : _a.remove();
        section_search.remove();
    }
});
// funcion sug_btn (botones que tienen las sugerencias de busqueda)
function sug_btn_function(event) {
...
function search_tag(tag, array, div) {
    let value_in_input = search_input.value;
    let array_new_productos = [];
