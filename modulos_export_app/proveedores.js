...
btn_create_proveedor.addEventListener("click", function (e) {
    ...
    console.log(obj);
    database_connect_proveedores.query('INSERT INTO proveedores SET ?', obj, function (error, results, fields) {
        if (error) {
            throw error;
        }
    });
...
//  class proveedores
class proveedores {
    constructor(id, nombre, telefono, email, descripcion) {
        this.Id = id;
        this.Nombre = nombre;
...
function get_proveedores() {
    database_connect_proveedores.query('SELECT * FROM `proveedores`', function (error, results, fields) {
        if (error) {
            throw error;
        }
        else {
            for (let index = 0; index < results.length; index++) {
                let prov = new proveedores(results[index].id, results[index].nombre, results[index].telefono, results[index].email, results[index].descripcion);
                create_proveedores(prov);
            }
        }
    });
}
function create_proveedores(proveedores) {
    let section_proveedores = document.getElementById('proveedores_list');
    ...
    btn_delete.addEventListener("click", function (e) {
        e.preventDefault();
        database_connect_proveedores.query(`DELETE FROM proveedores WHERE id = ${number_id}`, (error, results, fields) => {
            if (error) {
                throw error;
            }
        });
